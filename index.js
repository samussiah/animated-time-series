(function (global, factory) {
    ...
    function update(settings) {
      // variable mapping
      if (settings.stratification_var === null) settings.stratificaton_var = settings.id_var;
      if (settings.color_var === null) settings.color_var = settings.stratification_var; // variable labels

      if (settings.varLabels.stratification === null) settings.varLabels.stratification = settings.stratification_var; // x

      if (!['ordinal', 'discrete'].includes(settings.xType)) settings.xType = 'ordinal';
      if (settings.xType === 'ordinal') settings.xVar = 'visit';else if (settings.xType === 'discrete') settings.xVar = 'timepoint'; // y

      if (settings.yLabel === null) settings.yLabel = "".concat(settings.varLabels[settings.yVar], " - ").concat(settings.aggregateLabels[settings.aggregate]);
      if (!['mean', 'geomean'].includes(settings.aggregate)) settings.displayCIs = false;
      if (settings.alpha <= 0 | settings.alpha >= 1) settings.alpha = 0.05;
      if (settings.displayCIs === true) settings.yLabel = "".concat(settings.yLabel, " (").concat((1 - settings.alpha) * 100, "% CI)"); // stratification and color
    ...
    function settings() {
      ...
      return _ref = {
        ...
        displayCIs: true,
        // applies only to mean and geometric mean
        alpha: 0.05,
        ...
        timepoint: 0,
        measureIndex: 0
      }, _defineProperty(_ref, "measureOrder", []), _defineProperty(_ref, "speed", 1000), _defineProperty(_ref, "pause", 5000), _defineProperty(_ref, "width", null), _defineProperty(_ref, "widthFactor", 2), _defineProperty(_ref, "height", null), _defineProperty(_ref, "heightFactor", 2), _defineProperty(_ref, "margin", {
        top: 50,
        right: 100,
        bottom: 100,
        left: 75
      }), _defineProperty(_ref, "footnotes", [//`<sup>1</sup> Displaying [aggregate] [outcome].`,
      ]), _ref;
    }
    ...
    function mutate(data, settings) {
      ...
      var cleansed = data.map(function (d) {
        var datum = {}; // Rename data variables.

        Object.keys(settings).filter(function (setting) {
          return /_var$/.test(setting);
        }).forEach(function (setting) {
          datum[setting.replace(/_var$/, '')] = ['visit_order_var', 'day_var', 'result_var' //'baseline_var',
          //'change_var',
          //'percent_change_var',
          ].includes(setting) ? parseFloat(d[settings[setting]]) : d[settings[setting]];
        });
        return datum;
      }).filter(function (d) {
        return !isNaN(d.result);
      }).sort(function (a, b) {
        var measureSort = a.measure < b.measure ? -1 : b.measure < a.measure ? 1 : 0;
        var visitSort = a.visit_order - b.visit_order;
        var stratumSort = a.stratification < b.stratification ? -1 : b.stratification < a.stratification ? 1 : 0;
        var idSort = a.id < b.id ? -1 : b.id < a.id ? 1 : 0;
    ...
    function set(data, settings) {
      var set = {};
      set.id = create('id', data.cleansed);
      set.stratification = create('stratification', data.cleansed);
      set.color = create('color', data.cleansed);
      set.visit = create('visit', data.cleansed);
      set.visit_order = create('visit_order', data.cleansed);
      set.day = create('day', data.cleansed);
      set.measure = create('measure', data.cleansed); // Sort measures.

      if (settings.measureOrder !== null && Array.isArray(settings.measureOrder)) set.measure.sort(function (a, b) {
        var aIndex = settings.measureOrder.findIndex(function (measure) {
          return measure === a;
        });
        var bIndex = settings.measureOrder.findIndex(function (measure) {
          return measure === b;
        });
        return ~aIndex && ~bIndex ? aIndex - bIndex : ~aIndex || ~bIndex ? -1 : a < b ? -1 : b < a ? 1 : 0;
      });
      set.strata = set.stratification.reduce(function (curr, prev) {
        var ids = data.raw.filter(function (d) {
          return d[settings.stratification_var] === prev;
        }).map(function (d) {
          return d[settings.id_var];
        });
      ...
      set.timepoint = set.visit.map(function (visit) {
        return d3.median(data.cleansed.filter(function (d) {
        ...
        set.stratification.forEach(function (stratum, i) {
          ...
          set.visit.forEach(function (visit, j) {
            ...
            if (visitDatum === undefined) {
              ...
              visitDatum = j > 0 ? _toConsumableArray(stratumDatum[1][j - 1]) : _toConsumableArray(stratumDatum[1][j]);
    ...
    function nest(data, set, settings) {
      ...
      function (d) {
        ...
        return set.measure.indexOf(a[0]) - set.measure.indexOf(b[0]);
      }); // Identify and generate missing combinations of outcome / stratum / timepoint.

      var imputed = impute(nested, set);
      return imputed;
    }
    ...
    function canvas(key, dimensions) {
      ...
      var mainTransition = main.style('width', '0%').transition().duration(this.settings.speed).style('width', '50%'); //.on('end', transitionEnd);
    ...
    function xAxis(canvas, xScale, dimensions, type, set) {
      ...
      if (this.settings.rotateXTickLabels) xAxis.selectAll('text').style('text-anchor', 'end').attr('transform', 'rotate(-45)').attr('dx', '-.8em').attr('dy', '.15em');
      xAxis.label = canvas.append('g').call(function (g) {
        g.append('text').attr('class', 'axis-label').attr('text-anchor', 'middle').attr('font-size', 16).attr('font-weight', 'bold').attr('x', dimensions.widthAdj / 2).attr('y', dimensions.heightAdj + dimensions.margin.bottom - 32).text(_this.settings.xLabel);
      });
      return xAxis;
    }
    ...
    function yAxis(canvas, yScale, dimensions) {
      ...
      yAxis.gridLines = canvas.append('g').call(function (g) {
        g.attr('class', 'grid-lines').selectAll('line').data(yScale.ticks()).join('line').attr('x1', 0).attr('x2', dimensions.widthAdj).attr('y1', function (d) {
          return yScale(d);
        }).attr('y2', function (d) {
          return yScale(d);
        });
      });
      yAxis.label = canvas.append('g').call(function (g) {
        g.append('text').attr('class', 'axis-label').attr('transform', 'rotate(-90)').attr('text-anchor', 'middle').attr('font-size', 16).attr('font-weight', 'bold').attr('x', -dimensions.heightAdj / 2).attr('y', -(dimensions.margin.left - 32)).text(_this.settings.yLabel);
      });
      return yAxis;
    }
    ...
    function plotPoints(svg, data, scales) {
      ...
      points.attr('cx', function (d) {
        ...
        return scales.y(d[1].value);
      }).attr('stroke', 'white').attr('r', 0).transition().duration(this.settings.speed / 2).attr('r', this.settings.pointRadius);
      return pointGroups;
    }
    ...
    function plotCIs(svg, data, scales) {
      ...
      CIs.attr('x1', function (d) {
        ...
        return scales.y(d[1].value);
      }).attr('stroke-linecap', 'round').transition().duration(this.settings.speed / 4).delay(this.settings.speed / 2).attr('y1', function (d) {
    ...
    function plotAnnotations(svg, data, scales) {
      ...
      annotations.attr('x', function (d) {
        ...
        return d.y;
      }).attr('dx', this.settings.offset * this.data.set.stratification.length / 2 + 3).attr('dy', this.settings.fontSize / 3).attr('fill', function (d) {
        return d.color;
      }).style('font-weight', this.settings.fontWeight).text(function (d) {
        return d.text;
      }).attr('font-size', 0);
      annotations.transition().duration(this.settings.speed / this.data.set.stratification.length).delay(function (d, i) {
