(function (global, factory) {
      ...
      nested.forEach(function (measure, i) {
        ...
        set.stratification.forEach(function (stratum, i) {
          ...
          set.visit.forEach(function (visit, j) {
            ...
            visitDatum.timepoint = set.timepoint[visitDatum.index]; // Attach stratum-level data to visit.
    ...
    function getXScale(domain, dimensions) {
      var xScale = d3.scalePoint().domain(domain).range([0, dimensions.width]).padding([0.5]); //.tickValues(set.timepoint)
      //.tickFormat((d,i) => set.visit[i]);

      console.log(domain); // TODO: linear x-scale with tickValues? and tickFormat function
    ...
    function addXAxis(svg, xScale) {
      var visits = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
      var xAxis = svg.append('g').classed('atm-axis', true).attr('transform', 'translate(0,' + svg.dimensions.height + ')').call(d3.axisBottom(xScale));
      if (visits !== null) xAxis.selectAll('.tick').classed('atm-missing', function (d) {
    ...
    function plot() {
      ...
      var xScale = getXScale(this.set.visit, dimensions); // TODO: pass set in here
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          measure.xAxis = addXAxis(layout.svg, measure.scales.x, measure.visits);
          ...
          if (this.settings.annotate) measure.annotations = plotAnnotations.call(this, layout.svg, data, measure.scales);
