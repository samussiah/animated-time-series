(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ? (module.exports = factory(require('slim-select')))
        : typeof define === 'function' && define.amd
        ? define(['slim-select'], factory)
        : ((global = typeof globalThis !== 'undefined' ? globalThis : global || self),
          (global.animatedTimeSeries = factory(global.SlimSelect)));
})(this, function (SlimSelect) {
    'use strict';
    ...
    function drawLines(measure) {
        ...
        var lines = measure.layout.lines
            .selectAll('path')
            .data(
                measure.ids.map(function (d) {
                    return d[1];
                })
            )
            .join('path')
    ...
    function points(measure) {
        ...
        var groups = measure.layout.points
            .selectAll('g')
    ...
    function attr$1(measure, lines) {
        ...
        lines
            .attr('x1', function (d, i) {
                return _this.settings.x_type === 'ordinal'
                    ? measure.xScale(_this.set.visit[i]) + measure.xScale.bandwidth() / 2
                    : measure.xScale(_this.set.timepoint[i]);
            })
            .attr('x2', function (d, i) {
                return _this.settings.x_type === 'ordinal'
                    ? measure.xScale(_this.set.visit[i]) + measure.xScale.bandwidth() / 2
    ...
    function linesAggregate$1(measure) {
        var lines = measure.layout.linesAggregate
    ...
    function pointsAggregate$1(measure) {
        var points = measure.layout.pointsAggregate
            .append('circle')
            .datum(measure.aggregate)
    ...
    function layout$1(measure, key) {
        ...
        var lines = this.util
            ...
            .attr('clip-path', 'url(#'.concat(keyClass, ')'));
        var points = this.util
            .addElement('points', canvas, 'g')
            .attr('clip-path', 'url(#'.concat(keyClass, ')'));
        var linesAggregate = this.util
            .addElement('lines-aggregate', canvas, 'g')
            .attr('clip-path', 'url(#'.concat(keyClass, ')'));
        var pointsAggregate = this.util
