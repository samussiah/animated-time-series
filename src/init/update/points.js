...
export default function updatePoints(measure) {
    ...
    measure.groups.each(function (data, i, j) {
        ...
        const pair = data[1].filter((d) =>
            [main.timepoint.visit, main.timepoint.previous.visit].includes(d.visit)
        );
        ...
        if (pair.length === 2) {
            ...
            const point = g.append('circle').classed('atm-circle--transition', true).datum(datum1);
